...
<body>
    ...
    <div id="container-main">
        ...
        <?php
            ...
            // DB Query
            $sql = "SELECT * FROM schedule WHERE movieId = '$movie_id'";
            $result = $conn->query($sql);

            if ($result->num_rows > 0) {
                $i = 1;
                echo "<table><th>번호</th><th>날짜</th><th>시간</th>";
                while($row = $result->fetch_assoc()) {
                    echo "<tr><td>".$i++."<td>".$row["date"]."</td><td>".$row["time"]."</td></tr>";
                }
                echo "</table>";
            } else {
                echo "<p style='
        ...
        ?>
        <div class="menu">
            <a href="./manage_movie.php">[목록으로]</a>
            <a href="add_showtime.php?id=<?php echo $movie_id ?>">[상영 정보 추가]</a>
            <a href="delete_showtime.php?id=<?php echo $movie_id ?>">[상영 정보 삭제]</a>
        </div>
    </div>
